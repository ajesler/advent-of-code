...
Record = Struct.new(:range, :character, :password) do
  def valid?
    range.cover?(password.count(character))
  end

  def toboggan_valid?
    positions = [password[range.first-1], password[range.last-1]]
    positions.count { |c| c == character } == 1
  end

  def to_s
    "#{range.first}-#{range.last} #{character}: #{password} | v=#{valid?}, tv=#{toboggan_valid?}"
  end
end

# 4-7 h: hhxhzmwhhhh
class Parser
  MATCHER = /(?<range_start>\d+)-(?<range_end>\d+)\s(?<character>\w):\s(?<password>\w+)/.freeze
  ...
  def parse
    @lines
      ...
      .map { |md| [md[:range_start].to_i, md[:range_end].to_i, md[:character], md[:password]] }
      .map { |rs, re, c, p| Record.new(rs..re, c, p) }
  end
end
...
if $PROGRAM_NAME == __FILE__
  ...
  if first_argument == '--specs'
    ...
    lines = data_source.split(/\r?\n/)
    records = Parser.new(lines).parse
    # puts records.first(10) { |r| puts r.to_s }

    puts "Part 1: #{records.count(&:valid?)} valid passwords"
    puts "Part 2: #{records.count(&:toboggan_valid?)} valid passwords"
